#!/usr/bin/env python
"""Build the Dataset A candidate universe.

Parses the Ensembl GRCh38 annotation, reads the Open Targets gold-standard loci and
assigns every gene within +/-500 kb of each sentinel variant to its locus, with the
gold-standard gene labelled 1 and all competitor genes labelled 0.

Outputs: data/processed/ensembl_genes_grch38.parquet,
         data/processed/gold_standard_candidate_universe.parquet,
         data/processed/gold_standard_loci_summary.parquet
"""
...
def build_candidate_universe(df_genes: pd.DataFrame, gs_records: list) -> tuple:
    ...
    for locus_id, loc in unique_loci.items():
        ...
        # ranking requires >= 2 candidate genes and >= 1 positive
...
def write_audit_report(df_universe: pd.DataFrame, df_summary: pd.DataFrame, report_path: Path):
    ...
    report = f"""# Candidate Gene Universe & Label Audit Report
...
|---|---|---|
| **Total Valid Training Loci ($N$)** | **{total_loci:,}** | ** PASSED** ($\ge 100$ loci required; {total_loci/100:.1f}× threshold) |
...
def main():
    log.info("Starting Candidate Universe construction...")
    ...
    report_path = RESULTS_DIR / "candidate_universe_summary.md"
