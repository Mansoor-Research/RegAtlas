#!/usr/bin/env python
"""
Investigation: Distance-Matched rE2G Analysis & High-Confidence Sensitivity Audit.

Addresses the critical question:
"Among genes with similar distances from the GWAS locus, does rE2G still distinguish the supported gene?"

Analyses performed:
1. Distance Stratification / Binned Comparison:
   - Compares rE2G score, element count, and presence between Gold genes (Y=1) vs Competitors (Y=0)
     within strict distance bins (<50 kb, 50-100 kb, 100-250 kb, 250-500 kb).
2. Rank-Matched rE2G Comparison:
   - For genes sharing the exact same distance rank (e.g., Rank 1 nearest, Rank 2, Rank 3-5),
     evaluates whether rE2G is significantly higher in true positive genes.
3. Incremental Gain Test (Residual / Controlled Gain):
   - Compares Distance-only ranker vs Distance + rE2G ranker to quantify the exact marginal gain
     added by rE2G after controlling for distance.
4. High-Confidence Sensitivity Analysis:
   - Evaluates RegAtlas LambdaRank specifically on the subset of 511 High-Confidence Open Targets loci.
"""
...
def main():
    ...
    # Write Audit Report
    # -----------------------------------------------------------------------
    report_file = RESULTS_DIR / "distance_matched_re2g_and_sensitivity_audit.md"
