#!/usr/bin/env python
"""PGC3 concordance and Gene Ontology analysis of the schizophrenia rankings.

1. Compares top-ranked genes with the 120 PGC3 prioritized genes
   (Trubetskoy et al. 2022, Supplementary Table 12), against the nearest-TSS heuristic.
2. Runs g:Profiler GO enrichment (BP, CC, MF) for protein-coding top-ranked genes
   against a protein-coding candidate background (BH FDR < 0.05), with a
   sensitivity analysis excluding genes that are positive training labels.

Outputs: results/downstream_biology/
"""
...
def main():
    ...
    with open(report_file, "w", encoding="utf-8") as f:
        ...
        f.write("\n---\n\n## 3. Significant GO terms per analysis (FDR < 0.05)\n\n")
        for _name, _df in go_results.items():
            f.write(f"- `{_name}`: {len(_df)} significant terms\n")
