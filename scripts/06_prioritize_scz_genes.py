#!/usr/bin/env python
"""Prioritize schizophrenia candidate genes with the frozen RegAtlas model.

Trains the final model on all Dataset A loci (number of trees = median best
iteration from cross-validation), scores Dataset B and ranks genes within each
locus with a deterministic tie-break (see apply_model_to_scz()).

Outputs: data/processed/scz_prioritized_gene_rankings.{parquet,csv}
"""
...
def apply_model_to_scz(gbm: lgb.Booster, df_scz: pd.DataFrame) -> pd.DataFrame:
    ...
    # Deterministic ranking within each locus: higher score first; exact score ties
    # (2 of 111 loci) are broken by the is_nearest_tss flag and then by gene ID.
...
def analyze_scz_prioritization(df_scz: pd.DataFrame, rnaseq_dict: dict):
    ...
    print("\n" + "="*80)
    print("SCZ GENE PRIORITIZATION COMPLETE ")
    print("="*80)
...
def main():
    log.info("Starting Model Application & Prioritization...")
